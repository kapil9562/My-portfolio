          ...
          {projects.map((project, idx) => {
            ...
            return (
              <motion.div
                ...
                key={idx}
                viewport={{ once: true, amount: 0.4 }}
                className="w-full h-fit perspective-distant cursor-pointer bg-transparent group"
              >
                {/* 3D wrapper */}
                <div
                  className={`relative w-full h-fit transition-transform duration-700 ease-in-out transform-3d ${rotate[idx] ? "transform-[rotateY(180deg)]" : "transform-[rotateY(0deg)]"
                    }`}
                >

                  {/* BACK */}
                  <div
                    className={`absolute top-0 left-0 z-10 bg-black cursor-pointer h-full w-full flex flex-col justify-between rounded-2xl transition-all duration-300 backface-hidden transform-[rotateY(180deg)] ${rotate[idx] ? "opacity-100" : "opacity-0"
                      }`}
                  ...
                  >
                    ...
                    <div className='bg-black p-4 h-full w-full rounded-2xl flex flex-col justify-between'>
                      <div className="space-y-5">
                        <div className="w-full flex flex-row justify-between items-start">
                          ...
                          <button
                            ...
                            className="text-gray-100 hover:text-orange-400 lg:group-hover:pointer-events-auto lg:pointer-events-none lg:group-hover:opacity-100 lg:opacity-0 transition-opacity duration-200 p-1 rounded-lg font-medium cursor-pointer absolute top-3 right-3 z-50"
                            onClick={(e) => {
                              e.stopPropagation();
                              e.preventDefault();
                              toggleRotate(idx);
                          ...
                          >
                            <FaRotate className="text-lg hover:text-orange-400 transition-transform duration-500 active:rotate-180 " />
                          </button>
                        </div>

                        <div className="card_title__container space-y-4">
                          <h3 className="lg:text-3xl md:text-2xl sm:text-xl text-2xl font-bold bg-linear-to-r from-pink-500 to-orange-500 bg-clip-text text-transparent font-bricolage">
                            {project.title}
                          </h3>
                      ...
                      <div className="flex flex-row flex-wrap gap-x-4 gap-y-1 text-sm">
                        {project.tools.map((tool, i) => (
                          <span
                            className="text-orange-600 font-instrumentSans tracking-wide"
                            key={i}
                          >
                            {tool}
                          </span>
                      ...
                      <div className="absolute right-0 bottom-0 p-4">
                        <span className="text-[#FFFFFF15] font-semibold text-8xl font-bricolage">
                          {project.year}
                        </span>
                      </div>
                    </div>
                  ...
                  {/* FRONT */}
                  <div
                    className="card relative p-0! backface-hidden"
                    onClick={() => openProject(project.live)}
                  >
                    ...
                    <button
                      ...
                      aria-label={`Flip ${project.title} card`}
                      title={`Flip ${project.title} card`}
                      className="text-gray-100 hover:text-orange-400 bg-gray-800/60 lg:group-hover:pointer-events-auto lg:pointer-events-none lg:group-hover:opacity-100 lg:opacity-0 transition-opacity duration-200 p-1 rounded-lg font-medium cursor-pointer absolute top-3 right-3 z-50"
                      onClick={(e) => {
                        e.stopPropagation();
                        toggleRotate(idx);
                      }}
                    ...
                    <div className="w-full overflow-hidden rounded-t-2xl">
                      <img
                        ...
                        src={project.image}
                        alt="Thumbnail"
                        className="h-auto w-full object-cover align-top group-hover:scale-110 transition-transform duration-2000"
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-4 p-4">
                      <div className="card_title__container space-y-4">
                        <h3 className="md:text-2xl text-xl font-bold text-gray-100 font-bricolage">
                          {project.title}
                        </h3>
                      </div>
                      ...
                      >
                        <FaArrowUpRightFromSquare className='pointer-events-none' />
                        <span className='pointer-events-none'>Explore</span>
                      </button>
                    </div>
                  </div>
                </div>
